...
public:
    ...
    Gdft();
    //Class Constructor
    Gdft(uword a, uword b, Col<complex<T1>> &k1, Col<complex<T2>> &k2, Col<complex<T2>> &k3) //Change these argumenst as you need to setup the object
    {
        n1 = a;
        n2 = b;
        kx = k1;
    ...
    Col<complex<T1>> operator*(const Col<complex<T1>>& d) const//Don't change these arguments
    {
        ...
        Col<T2> ky_r = imag(this->ky);
        Col<T2> kz_r = imag(this->kz);
        ...
        Col<T2> realXformedData(realDataXformPtr, dataLength);
        Col<T2> imagXformedData(imagDataXformPtr, dataLength);
