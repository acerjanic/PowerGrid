...
operator*(const Col<complex<T1>> &d) const // Don't change these arguments
{
    ...
    computeFd_CPU_Grid(n2, kx, ky, kz, dataPtr,
                            ...
                            stream, nPlan, pGridData, pGridData_d, pGridData_os,
                            pGridData_os_d, pSamples);
    pgCol<CxT1> testGPUResult(reinterpret_cast<CxT1 *>(pSamples), n2);
    //Col<CxT1> temp(reinterpret_cast<CxT1 *>(pSamples), n2, false, true);
    Col<CxT1> temp = testGPUResult.getArma();
    return temp; // Return a vector of type CxT1
}

template <typename T1>
inline pgCol<complex<T1>> Gnufft<T1>::
operator*(const pgCol<complex<T1>> &d) const // Don't change these arguments
{
    RANGE(__PRETTY_FUNCTION__)
    ...
    pgCol<CxT1> testGPUResult(reinterpret_cast<CxT1 *>(pSamples), n2);
    return testGPUResult; // Return a vector of type CxT1
}


// Adjoint transform operation
template <typename T1>
inline pgCol<complex<T1>> Gnufft<T1>::operator/(const pgCol<complex<T1>> &d) const {
    // uword dataLength = n2;
    ...
    pgCol<CxT1> temp(reinterpret_cast<CxT1 *>(pGridData), n1);
    return temp; // Return a vector of type T1
}
...
inline Col<complex<T1>> Gnufft<T1>::operator/(const Col<complex<T1>> &d) const {
    ...
    pgCol<CxT1> testGPUResult(reinterpret_cast<CxT1 *>(pGridData), n1);
    Col<CxT1> temp = testGPUResult.getArma();
    return temp; // Return a vector of type T1
}
