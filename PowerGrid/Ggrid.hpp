...
public:
    ...
    Col<CxT1> operator/(const Col<CxT1>& d) const
    {
        ...
        //Basically if we know that the data points are zero, they have no impact on the transform
    ...
    Col <CxT1> trimmedForwardOp(const Col <CxT1> &d, const Col <CxT1> &tempInterp) const//Don't change these arguments
    {
        //Let's trim the operations to avoid data overhead and transfers
        //Basically if we know that the data points are zero, they have no impact on the transform
        uword dataLength = this->n2;
        ...
        //std::cout << "Length of DataMaskTrimmed = " << dataLengthTrimmed << std::endl;
        ...
        //cout << "About to call the forward gridding routine." << endl;
        computeFd_CPU_Grid<T1>(dataLengthTrimmed, kxTrimmed.memptr(), kyTrimmed.memptr(), kzTrimmed.memptr(),
                               realDataPtr, imagDataPtr, Nx, Ny, Nz,
                               gridOS, realXformedDataPtr, imagXformedDataPtr, kernelWidth, beta, LUT, sizeLUT);
    ...
    //Adjoint transform operation
    Col <CxT1> trimmedAdjointOp(const Col <CxT1> &d, const Col <CxT1> &tempInterp) const {
        ...
        computeFH_CPU_Grid<T1>(dataLengthTrimmed, kxTrimmed.memptr(), kyTrimmed.memptr(), kzTrimmed.memptr(),
                               realDataPtr, imagDataPtr, Nx, Ny, Nz,
                               gridOS, realXformedDataPtr, imagXformedDataPtr, kernelWidth, beta, LUT, sizeLUT);
        /*
        iftCpu<T2>(realXformedDataPtr,imagXformedDataPtr,
